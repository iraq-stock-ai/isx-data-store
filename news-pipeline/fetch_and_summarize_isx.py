...
def summarize_with_gemini(title: str, raw_text: str, max_retries: int = 3) -> dict:
    ...
    prompt_text = (
        ...
        "⚠️ تحذير مهم جداً قبل الاستخراج: بعض هذه الإفصاحات صادرة من شركات "
        "وساطة مالية (مثل شركات السمسرة)، وتحتوي ترويسة الخطاب أحياناً على "
        "معلومات مالية تخص شركة الوساطة نفسها (مثل رأس مالها المذكور بجانب "
        "شعارها) - وهذه معلومات لا علاقة لها بالشركة موضوع الإفصاح الفعلي. "
        "يجب عليك التمييز بدقة تامة بين:\n"
        "  (أ) الشركة موضوع الإفصاح الفعلي (التي يخصها القرار أو الحدث المُعلَن)\n"
        "  (ب) أي شركة وساطة أو طرف ثالث مذكور فقط كناقل أو منفّذ للإجراء\n"
        "استخرج الأرقام المالية فقط إذا كانت تخص صراحة الشركة موضوع الإفصاح "
        "(أ)، وليس أي شركة أخرى مذكورة بالنص لسبب إجرائي بحت.\n\n"
